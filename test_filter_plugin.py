...
class TestFilterPlugin(unittest.TestCase):
    ...
    def test_plugin_initialization(self):
        ...
        self.assertIn("enable_text_cleaning", self.plugin.config)
        self.assertIn("enable_exact_match", self.plugin.config)
        self.assertIn("enable_unique_filtering", self.plugin.config)
    ...
    def test_filter_posts_by_keyword(self):
        """Тест фильтрации по ключевому слову без очистки текста"""
        posts = [
            ...
            {"text": "Обычный пост без ключевых слов", "owner_id": 3, "id": 300},
        ]
        
        # Фильтрация по первому ключу
        filtered = self.plugin.filter_posts_by_keyword(posts, "Даже в боевых условиях", exact_match=True)
        self.assertEqual(len(filtered), 1)
        self.assertEqual(filtered[0]["owner_id"], 1)
        
        # Фильтрация по второму ключу
        filtered = self.plugin.filter_posts_by_keyword(posts, "Лис передает", exact_match=True)
        self.assertEqual(len(filtered), 1)
        self.assertEqual(filtered[0]["owner_id"], 2)
        
        # Фильтрация по несуществующему ключу
        filtered = self.plugin.filter_posts_by_keyword(posts, "Несуществующий ключ", exact_match=True)
