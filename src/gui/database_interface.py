...
class DatabaseInterface:
    ...
    def setup_ui(self):
        ...
        # Заголовок
        ttk.Label(self.frame, text="База данных результатов парсинга", 
    ...
    def create_tasks_panel(self):
        ...
        ttk.Button(btn_frame, text="Экспорт выбранной", command=self.export_selected_task).pack(side="left", padx=(0, 5))
        ttk.Button(btn_frame, text="Удалить задачу", command=self.delete_selected_task).pack(side="left")
        
        # Таблица задач
        columns = ("ID", "Название", "Статус", "Постов", "Лайков", "Комментариев", "Репостов", "Просмотров", "SI", "Дата")
