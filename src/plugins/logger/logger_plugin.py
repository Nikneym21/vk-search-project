...
class LoggerPlugin(BasePlugin):
    def __init__(self):
        ...
        self.config = {
            ...
            "rate_limit": {"info": 10, "warning": 5, "error": 3, "debug": 20},  # максимум логов в секунду
            "filter_patterns": [
    ...
    def _batch_log(self, level: str, message: str):
        ...
        current_time = time.time()
        self.batch_logs[level].append({"message": message, "timestamp": current_time})

        # Проверяем, нужно ли вывести batch
        if (
            len(self.batch_logs[level]) >= self.config["batch_size"]
            or current_time - self.last_batch_time[level] >= self.config["batch_timeout"]
        ):
    ...
    def _print_log_summary(self):
        ...
        logger.info(
            f"📊 Сводка логов: {summary['total_logs']} всего "
