#!/usr/bin/env python3
"""
Плагин для управления базой данных результатов парсинга
"""
...
class DatabaseManagerPlugin(BasePlugin):
    """Плагин для управления базой данных результатов парсинга"""
    
    def __init__(self):
        super().__init__()
        self.name = "DatabaseManagerPlugin"
        self.db_path = "data/parser_results.db"
        self.connection = None
        
    def initialize(self):
        """Инициализация плагина и создание таблиц"""
        self.log_info("Инициализация плагина управления базой данных")
        ...
        try:
            ...
            self.log_info("Плагин управления базой данных инициализирован")
            return True
            ...
            for post in posts:
                ...
                try:
                    cursor.execute('''
                        ...
                        (task_id, vk_id, text, text_hash, date, likes, comments, reposts, views, keywords_matched)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (
                        task_id,
                        vk_id,
    ...
    def export_task_to_csv(self, task_id: int, output_path: str) -> bool:
        ...
        try:
            ...
            for post in posts:
                csv_row = {
                    'link': f"https://vk.com/wall{post['vk_id']}",
                    'text': post['text'],
                    'type': 'Пост',
                    'author': '',
                    'author_link': '',
    ...
    def find_duplicates(self, task_id: int = None) -> List[List[Dict]]:
        """Поиск дубликатов постов"""
        try:
            cursor = self.connection.cursor()
            
            if task_id:
                cursor.execute('''
                    SELECT text_hash, COUNT(*) as count, GROUP_CONCAT(id) as post_ids
                    FROM posts 
                    WHERE task_id = ? 
                    GROUP BY text_hash 
                    HAVING COUNT(*) > 1
    ...
    def shutdown(self):
        ...
        self.log_info("Завершение работы плагина управления базой данных")
        if self.connection:
