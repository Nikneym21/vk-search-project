...
class MonitoringPlugin(BasePlugin):
    ...
    def initialize(self) -> None:
        ...
        # Запускаем мониторинг безопасно
        if self.config["enabled"]:
            self._start_monitoring_safe()
            self.log_info("Мониторинг инициализирован")

        self.emit_event(EventType.PLUGIN_LOADED, {"status": "initialized"})

    def _start_monitoring_safe(self):
        """Безопасный запуск мониторинга с проверкой event loop"""
        try:
            ...
            if loop.is_running():
                ...
                self.log_info("Задача мониторинга создана в активном event loop")
            else:
                # Если loop не активен, отложим запуск
                self.log_info("Event loop не активен, мониторинг будет запущен позже")
                self.monitoring_task = None
        except RuntimeError as e:
            # Если event loop отсутствует, отложим запуск
            self.log_warning(f"Event loop недоступен: {e}. Мониторинг будет запущен позже")
            self.monitoring_task = None

    def start_monitoring_if_needed(self):
        """Запускает мониторинг, если он еще не запущен и есть активный event loop"""
        if self.config["enabled"] and self.monitoring_task is None:
            try:
                loop = asyncio.get_event_loop()
                if loop.is_running():
                    self.monitoring_task = loop.create_task(self._monitoring_loop())
                    self.log_info("Мониторинг запущен отложенно")
                    return True
            except RuntimeError:
                pass
        return False
    ...
    async def _collect_metrics(self):
        ...
        try:
            ...
            if vk_plugin:
                ...
                if vk_stats:
                    self.metrics["response_times"] = vk_stats.get("performance_metrics", {}).get(
            ...
            self.log_info(
                f"📊 Метрики собраны: RT={self.metrics['response_times']:.3f}s, "
    ...
    async def _check_alerts(self):
        ...
        if (
            isinstance(self.metrics["response_times"], (int, float))
            and self.metrics["response_times"] > thresholds["response_time"]
        ):
            alerts.append(f"⚠️ Высокое время ответа: {self.metrics['response_times']:.2f}s")

        # Проверка rate limit hits
        if (
            isinstance(self.metrics["rate_limit_hits"], (int, float))
            and self.metrics["rate_limit_hits"] > thresholds["rate_limit_hits"]
        ):
            alerts.append(f"🚨 Много rate limit hits: {self.metrics['rate_limit_hits']}")

        # Проверка cache hit rate
        if (
            isinstance(self.metrics["cache_hit_rate"], (int, float))
            and self.metrics["cache_hit_rate"] < thresholds["cache_hit_rate"]
        ):
            alerts.append(f"📉 Низкий cache hit rate: {self.metrics['cache_hit_rate']:.1%}")

        # Проверка использования памяти
        if (
            isinstance(self.metrics["memory_usage"], (int, float))
            and self.metrics["memory_usage"] > thresholds["memory_usage"]
        ):
        ...
        for alert in alerts:
            ...
            self.alerts.append({"timestamp": datetime.now().isoformat(), "message": alert, "level": "warning"})
    ...
    def _save_metrics_history(self):
        ...
        history_entry = {"timestamp": datetime.now().isoformat(), "metrics": self.metrics.copy()}
